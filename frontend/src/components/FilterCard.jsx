...
const FilterCard = ({ isVisible, onClose }) => {
  ...
  return (
    <div className="fixed z-50 w-64 overflow-hidden bg-white border border-gray-100 shadow-lg top-20 left-5 rounded-xl animate-fadeIn">
      <div className="p-4 bg-gradient-to-r from-indigo-600 to-indigo-700">
        <div className="flex items-center justify-between">
          ...
          <button 
            onClick={onClose}
            className="p-1 transition-colors rounded-full hover:bg-indigo-800"
          >
        ...
        <RadioGroup value={selectedValue} onValueChange={changeHandler}>
          {filterData.map((data, index) => (
            <div key={index} className="mb-6">
              <h2 className="mb-3 text-xs font-semibold tracking-wider text-indigo-600 uppercase">{data.fitlerType}</h2>
